...
async function downloadCV() {
  ...
  try {
    ...
    const experiencia = document.getElementById(
      "preview-experiencia"
    ).innerText;
    ...
    doc.text(nombre, centerX, y);
    y += 12; // espacio debajo del nombre

    // Agregar imagen centrada
    if (imgData) {
      const imgWidth = 40;
      const imgX = (pageWidth - imgWidth) / 2;
      doc.addImage(imgData, "JPEG", imgX, y, imgWidth, 40);
      y += 45; // espacio debajo de la imagen
    }
    doc.setFontSize(11);
    ...
    y += 12; // Espacio extra después del encabezado
    ...
    y += perfilLines.length * 6 + 8; // +8 para espacio extra
    ...
    doc.setFontSize(11);
    // Supongamos que habilidades es un string separado por comas
    const habilidadesArray = habilidades.split(" ").map((h) => h.trim());
    const habLines = [];
    habilidadesArray.forEach((habilidad) => {
      habLines.push(...doc.splitTextToSize(habilidad, 180));
    });
    doc.text(habLines, margin, y);
    y += habLines.length * 6 + 8;
...
function analyzeResponse(response, questionType) {
  ...
  switch (questionType) {
    case "strengths":
      ...
      if (
        ...
        suggestions.push(
          "Agrega un ejemplo específico de cuándo usaste esa fortaleza"
        );
      }
      break;
    ...
    case "pressure":
      if (
        text.includes("organizo") ||
        text.includes("priorizo") ||
        text.includes("planif")
      ) {
        score += 15;
        feedback += "✅ **Estrategia:** Bien, mencionas técnicas específicas. ";
      } else {
        suggestions.push(
          "Menciona técnicas concretas como organización o priorización"
        );
      }
      ...
      if (
        ...
        score += 10;
        feedback +=
          "✅ **Resultados:** Perfecto, hablas de outcomes positivos. ";
      } else {
    ...
    case "future":
      if (
        ...
        score += 15;
        feedback +=
          "✅ **Crecimiento:** Excelente, muestras mentalidad de crecimiento. ";
      } else {
      ...
      if (
        ...
        text.includes("organización") ||
        text.includes("equipo")
      ) {
        score += 10;
        feedback +=
          "✅ **Compromiso:** Bien, conectas tu futuro con la empresa. ";
      } else {
        suggestions.push(
          "Conecta tus objetivos con el crecimiento de la empresa"
        );
      }
      break;
    ...
    case "value":
      ...
      if (
        ...
        text.includes("contribu")
      ) {
        score += 15;
        feedback += "✅ **Valor:** Perfecto, hablas de tu contribución. ";
      } else {
        suggestions.push(
          "Explica específicamente qué valor agregarías a la empresa"
        );
      }
      break;
    ...
    case "challenge":
      if (
        ...
        score += 10;
        feedback +=
          "✅ **Contexto:** Bien, describes la situación claramente. ";
      } else {
        suggestions.push("Describe mejor el contexto del desafío");
      }

      if (
        text.includes("solución") ||
  ...
  const questionKeywords =
    questions.find((q) => q.type === questionType)?.keywords || [];
  const keywordMatches = questionKeywords.filter(
    (keyword) => text.includes(keyword) || text.includes(keyword.slice(0, -1))
  );
  ...
  if (text.includes("tarea") || text.includes("responsabilidad"))
    starElements++;
  if (
    text.includes("hice") ||
    text.includes("realicé") ||
    text.includes("decidí")
  )
    starElements++;
  if (
    text.includes("resultado") ||
    text.includes("logré") ||
    text.includes("conseguí")
  )
    starElements++;
  ...
  if (score >= 80) {
    ...
    feedback +=
      "🔄 **Respuesta necesita trabajo.** Te sugiero rehacer la respuesta.";
  }
...
function submitResponse() {
  ...
  if (response.trim()) {
    ...
    // Analizar respuesta
    const feedback = analyzeResponse(
      response,
...
function actualizarPreview() {
  ...
  const email = document.getElementById("email").value || "tu@email.com";
  const telefono = document.getElementById("telefono").value || "Tu teléfono";
  const direccion =
    document.getElementById("direccion").value || "Tu dirección";
  const linkedin = document.getElementById("linkedin").value || "LinkedIn";
  const perfil =
    document.getElementById("perfil").value ||
    "Aquí aparecerá tu perfil profesional...";

  document.getElementById("preview-nombre").textContent = nombre;
  document.getElementById("preview-email").textContent = `Email: ${email}`;
  document.getElementById(
    "preview-telefono"
  ).textContent = `Teléfono: ${telefono}`;
  document.getElementById(
    "preview-direccion"
  ).textContent = `Dirección: ${direccion}`;
  document.getElementById(
    "preview-linkedin"
  ).textContent = `LinkedIn: ${linkedin}`;
  document.getElementById("preview-perfil").textContent = `Perfil: ${perfil}`;

  // Experiencia
  const experienciaContainer = document.getElementById("preview-experiencia");
  experienciaContainer.innerHTML = "";

  const experiencias = document.querySelectorAll(
    "#experiencia-container .dynamic-section"
  );

  // if (experiencias.length > 0) {
  //   const titulo = document.createElement("h3");
  //   titulo.textContent = "Experiencia Laboral";
  //   titulo.classList.add("section-title");
  //   experienciaContainer.appendChild(titulo);
  // }

  experiencias.forEach((section) => {
  ...
  const educaciones = document.querySelectorAll(
    "#educacion-container .dynamic-section"
  );
  //   if (educaciones.length > 0) {
  //     const titulo = document.createElement("h3");
  //     titulo.textContent = "🎓 Educación";
  //     educacionContainer.appendChild(titulo);
  //   }

  educaciones.forEach((section) => {
  ...
  const idiomas = document.querySelectorAll(
    "#idiomas-container .dynamic-section"
  );
  //   if (idiomas.length > 0) {
  //     const titulo = document.createElement("h3");
  //     titulo.textContent = "🌐 Idiomas";
  //     idiomasContainer.appendChild(titulo);
  //   }

  idiomas.forEach((section) => {
